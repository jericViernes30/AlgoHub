        ...
        <div class="w-full flex h-screen bg-[#F2EBFB]">
            ...
            <div class="w-full p-7">
                <div class="w-fit flex gap-2 items-center justify-center mb-5 text-sm">
                    ...
                    <p class="pt-[1px]">{{$course}}</p>
                </div>
                <div class="w-full flex items-center justify-between">
                    <p class="text-2xl font-medium">Course Details - {{$course}}</p>
                    <button onclick="confirmDelete('{{ $course }}')" class="py-2 px-5 bg-red-300 rounded-md text-xs {{session('role') === 'admin' ? '' : 'hidden'}}">Delete course</button>
                    <script>
                        function confirmDelete(course) {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    window.location.href = "{{ route('admin.course_delete', ['course' => '__COURSE__']) }}".replace('__COURSE__', course);
                                }
                            });
                        }
                    </script>
                </div>
                <p class="text-sm italic">{{$lessonCount}} lessons</p>
